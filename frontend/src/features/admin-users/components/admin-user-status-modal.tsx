...
interface AdminUserStatusModalProps {
    ...
    isOpen: boolean;
    onClose: () => void;
    onConfirm: (user: AdminUser, newStatus: boolean) => Promise<void>;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
            <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-zinc-900 dark:text-white">
                            {willDeactivate ? "Deactivate User Account" : "Activate User Account"}
                        </h3>
                ...
                <p className="mt-4 text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
                    {willDeactivate
                        ? `Are you sure you want to deactivate ${user.username}? Inactive users are immediately prevented from signing in and performing actions across the platform.`
                        : `Are you sure you want to activate ${user.username}? The user will regain normal access to the platform based on their assigned role (${user.role}).`}
                </p>
                ...
                <div className="mt-6 flex justify-end gap-3 border-t border-zinc-100 pt-4 dark:border-zinc-800">
                    ...
                    <button
                        type="button"
                        onClick={() => onConfirm(user, !user.isActive)}
                        disabled={isSubmitting}
